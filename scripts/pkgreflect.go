...
func main() {
	...
	flag.BoolVar(&nofuncs, "nofuncs", true, "Don't list package functions")
	flag.BoolVar(&novars, "novars", true, "Don't list package variables")
	flag.BoolVar(&noconsts, "noconsts", true, "Don't list package consts")
...
func parseDir(dir string) {
	...
	for _, pkg := range pkgs {
		...
		// Types
		if !notypes {
			fmt.Fprintln(&buf, "var nodeTypes = Types{")
			template := "\t{\n\t\tName:    \"%s\",\n\t\tFields:  \"" + pkg.Name + ".%s\",\n\t\tComment: \"\",\n\t},\n"
			output(&buf, pkg, ast.Typ, template)
			// print(&buf, pkg, ast.Typ, "\t\"%s\": reflect.TypeOf((*"+pkg.Name+".%s)(nil)).Elem(),\n")
			fmt.Fprintln(&buf, "}")
			fmt.Fprintln(&buf, "")
		}
...
func output(w io.Writer, pkg *ast.Package, kind ast.ObjKind, format string) {
	names := []string{}
...
func skip(name string) bool {
	skipObject := map[string]bool{
		...
		"SetPosition":    false,
		"Statement":      false,
		"StatementDepth": false,
		"Stmt":           false,
		"StmtVisitor":    false,
	}
	if _, ok := skipObject[name]; ok {
		return false
	} else {
		return true
	}
